...
![Форма ручного начисления бонусов](../assets/media/portal/bonuses_manual_ru.png)
...
`Начислить бонусы` отправляет заполненную форму на начисление. Вкладка `Импортировать файл` вместо одиночной формы открывает загрузку `.csv`; `Загрузите файл` вызывает выбор файла, а зона `Перетащите файл сюда` принимает перетаскивание.
