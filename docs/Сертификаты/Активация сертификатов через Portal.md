...
![Вкладка Одиночный до поиска](../assets/media/portal/certificate_activation_single_ru.png)

`Поиск` отправляет введённый 16-значный номер на проверку. `Сбросить` очищает поле и результат поиска. Эти кнопки не активируют сертификат сами по себе в начальном состоянии.
