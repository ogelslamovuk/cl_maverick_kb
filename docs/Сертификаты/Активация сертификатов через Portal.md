...
Сертификаты активируются через форму на портале `mooon.by`. Для сертификатов Maverick в поле `Компании` обязательно выбирается ООО "Кино Маяк" — так эмитент корректно попадает в учёт.
...
![Экран загрузки файла для активации сертификатов](../assets/media/certificates/portal_certificate_activation_upload.png)
...
1. Зайти на портал `mooon.by`.
2. Открыть `Администрация -> Активация сертификатов`.
...
Нижняя часть формы после загрузки файла:

![Нижняя часть формы активации сертификатов](../assets/media/certificates/portal_certificate_activation_form_fields.png)

Пример корректно заполненной формы перед сохранением:

![Пример заполненной формы активации сертификатов](../assets/media/certificates/portal_certificate_activation_filled_example.png)
...
- Сертификаты и учёт — high-risk зона.
