...
![manager sales table](../assets/media/manager/manager-sales-table.jpg)
...
![manager sale card](../assets/media/manager/manager-sale-card.jpg)
...
Если нужно понять, почему скидка сработала или не сработала, проверяй настройки программы, категории, объектов, правил и привилегий. См. [Программы лояльности в Manager](Программы%20лояльности%20в%20Manager.md).
...
Если вопрос связан с сертификатом, смотри также [Проверка и разбор проблем с сертификатами](../Сертификаты/Проверка и разбор проблем с сертификатами.md).

## Отчёты по продажам

Если нужна не карточка конкретной операции, а табличная сводка по продажам, используй раздел **Отчёты**. Для билетных продаж подходят отчёты **Продажи (билеты)** и **Admissions**, для товаров — **Продажи (товары)**, для общей управленческой проверки — **Продажи**.

Подробнее: [Отчёты в Manager](Отчёты%20в%20Manager.md).
...
- [Сертификаты в Manager](Сертификаты в Manager.md)
- [Проверка и разбор проблем с сертификатами](../Сертификаты/Проверка и разбор проблем с сертификатами.md)
